...
OUTPUT = Path(__file__).parent.parent / "sycophancy_lie_truth.json"
SEED = 42
...
for r in lies:
    dataset.append({
        ...
        "turn1_answer": r["parsed_turn1"],
        "turn2_answer": r.get("judge_turn2_answer", "").strip().upper(),
    })
...
for r in truths:
    dataset.append({
        ...
        "model_response_turn1": r["model_response_turn1"],
        "followup_message": r["followup_message"],
        "turn1_answer": extract_letter(r["model_response_turn1"]),
        "turn2_answer": extract_letter(r["model_response_turn2"]),
    })
