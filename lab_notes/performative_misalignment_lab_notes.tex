...
Since we have done many evaluations on the in-context alignment faking setup with \model{meta-llama/Llama-3.3-70B-Instruct}, we already have a grasp on the model's behavior in the evaluation and some insights into its reasoning traces.

From here on, I used the SDF setup (system prompt for SDF) and \model{meta-llama/Llama-3.3-70B-Instruct}.
...
\caption{Behavioral metrics on harmful behavior prompts (Direction~A vs.\ A+B, negative multiplier).}
