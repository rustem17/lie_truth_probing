"""
Validate covariance-corrected mass-mean probes on validation/control sets.

Loads pre-extracted activations and pre-trained probe directions, then computes
AUROC via dot product scoring on paired differences.
"""
...
def direction_from_probe(probe, score_mode):
    if score_mode == "saved":
        return probe["direction"], probe.get("score_mode", "saved")
    if score_mode == "feature":
        return probe.get("feature_direction", probe["direction"]), "feature"
    if score_mode == "iid":
        return probe.get("decision_direction", probe["direction"]), "iid"
    raise ValueError(f"unknown score_mode: {score_mode}")


def validate(data_dir="../..", activations_dir=None, probes_dir=".", model=DEFAULT_MODEL_TAG, score_mode="saved"):
    results = {}
    ...
    for probe_name, val_sets in VALIDATION_MAP.items():
        ...
        direction, effective_mode = direction_from_probe(probe, score_mode)
        direction = normalize(direction)
        best_layer = probe["best_layer"]

        print(f"\n{probe_name} probe (best layer {best_layer}, score_mode={effective_mode}):")
