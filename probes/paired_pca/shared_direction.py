"""
Shared paired-PCA direction analysis across deception conditions.

Per-dataset directions are first PCs of lie-minus-truth pair-difference
matrices. Shared directions are normalized averages of per-dataset PCs.
"""
...
def paired_pca_direction(D, center=False):
    mask = finite_rows(D)
    if not np.any(mask):
        return np.zeros(D.shape[1], dtype=D.dtype), 0.0
    D = D[mask]
    mu = D.mean(axis=0)
...
def train_all_directions(diffs, train_names, center=False):
    directions = {}
    power_ratios = {}
    for name in train_names:
        ...
        for layer in range(n_layers):
            direction, ratio = paired_pca_direction(D[:, layer], center=center)
            dirs.append(direction)
            ratios.append(ratio)
        directions[name] = np.stack(dirs)
        power_ratios[name] = ratios
        print(f"{name}: {n_pairs} pairs, {n_layers} layers")
    return directions, power_ratios
...
def analyze(data_dir="../..", activations_dir=None, output_dir=".", datasets=None, center=False, model=DEFAULT_MODEL_TAG):
    ...
    directions, power_ratios = train_all_directions(diffs, sorted(diffs.keys()), center=center)
    ...
    torch.save({
        ...
        "per_layer_power_ratios": power_ratios,
